...
///
/// This is the canonical set from CPython's `sys.stdlib_module_names`
/// (Python 3.14), regenerate with:
///
/// ```text
/// python3 -c "import sys; print(chr(10).join(sorted(sys.stdlib_module_names)))"
/// ```
///
/// It includes the private C-extension backing modules (leading `_`); they are
/// genuinely part of the standard library, so categorizing them as stdlib is
/// correct. Dotted submodules (e.g. `urllib.parse`) are matched by their root
/// segment in [`PackageRegistry::is_stdlib`](crate::registry::PackageRegistry::is_stdlib),
/// so they need not be listed individually.
